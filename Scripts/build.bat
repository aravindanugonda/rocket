...
REM You could add checks for PATH containing %COBDIR%\bin here if needed

REM Set log file
set logfile=%logdir%\Compile_%mydate%_%mytime%.log

REM Create directories if they don't exist (Consider moving this to YAML permissions step)
...
set _sub_rc=!errorlevel!  REM <-- Fix: Use !errorlevel!
...
if !_sub_rc! leq 8 (  REM <-- Fix: Use !_sub_rc!
...
exit /b %_sub_rc%  REM <-- Fix: Exit with captured code
...
set "COBCPY=%bms_cpy%;%cpy_dir%;%COBCPY%" # Use quotes for safety
echo Using COBCPY=!COBCPY!>> %logfile% # Use !COBCPY!
...
set _sub_rc=!errorlevel! # <-- Fix: Use !errorlevel!

REM Copy compiled files to execution directory if successful
if !_sub_rc! leq 8 ( # <-- Fix: Use !_sub_rc!
...
exit /b %_sub_rc% # <-- Fix: Exit with captured code

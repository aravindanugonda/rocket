...
rem Added commands to 'set' other important vars after successful call
...
rem --- Check temp file ---
if not exist "!TEMP_ENV_FILE!" ( /* ... error handling ... */ exit /b 2)
for %%F in ("!TEMP_ENV_FILE!") do if %%~zF==0 ( /* ... error handling ... */ if !CALL_EXITCODE! NEQ 0 if !CALL_EXITCODE! NEQ 1 ( exit /b 3 ) )
...
for /f "usebackq tokens=1* delims==" %%a in ("!TEMP_ENV_FILE!") do (
  rem Trim potential whitespace artifacts (basic trim)
...
REM echo LIB captured: !ED_LIB!
REM echo INCLUDE captured: !ED_INCLUDE!
REM echo CLASSPATH captured: !ED_CLASSPATH!
...
if "!COBDIR!"=="" ( echo ERROR: COBDIR was NOT captured. & exit /b 1 )
if "!ED_PATH!"=="" ( echo ERROR: PATH was NOT captured. & exit /b 1 )
REM Add checks for LIB/INCLUDE if critical

rem --- Cleanup COBDIR captured ---
set "COBDIR_CLEAN=!COBDIR:"=!"
if "!COBDIR_CLEAN:~-1!"==";" set "COBDIR_CLEAN=!COBDIR_CLEAN:~0,-1!"
